...
struct FAnyMesh {
	...
	inline bool setMesh(USkeletalMesh* mesh, UObject * outer) {
		isSkeletal = true;
		bool wasNewlyCreated = create(outer);
		skeletal->SetSkeletalMesh(mesh);
		if (ztatic != nullptr)ztatic->SetStaticMesh(nullptr);
		return wasNewlyCreated;
	}
	inline bool setMesh(UStaticMesh* mesh, UObject * outer) {
		isSkeletal = false;
		bool wasNewlyCreated = create(outer);
		if(skeletal!=nullptr)skeletal->SetSkeletalMesh(nullptr);
		ztatic->SetStaticMesh(mesh);
		return wasNewlyCreated;
	}
	...
	UPrimitiveComponent* findSocketInverted(FVector& location, FRotator& rotation, const FName itemSocket);
	...
	inline void attach(USkeletalMeshComponent* const characterMesh, const FName& itemSocket, const FName characterSocket) {
		FVector location;
		FRotator rotation;
		UPrimitiveComponent* comp = findSocketInverted(location, rotation, itemSocket);
		if (comp != nullptr) {
